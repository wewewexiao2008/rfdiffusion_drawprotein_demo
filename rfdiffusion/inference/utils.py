...
from rfdiffusion.util import rigid_from_3_points, calc_rmsd
from rfdiffusion.util_module import ComputeAllAtomCoords
from rfdiffusion import util
from rfdiffusion.curve_utils import assign_secondary_structures
import random
import logging
...
def get_mu_xt_x0(
    ...
    px0_ca = px0[:, 1, :]
    # xt_ca = torch.Tensor(
    #     np.concatenate((cX_ca, np.zeros((px0_ca.shape[0] - cX_ca.shape[0], 3))), axis=0)
    # )
    # print(f"xt_ca:{xt_ca.shape}")
    if not method == 5 and not method == 6:

        cX_coords = torch.Tensor(cX_ca).unsqueeze(0)
        # xt_SSE_list = assign_secondary_structures(
        #     xt_ca[: cX_ca.shape[0], :].unsqueeze(0), return_encodings=False, full=False
        # )
        xt_SSE_onehot = assign_secondary_structures(
            xt_ca[: cX_ca.shape[0], :].unsqueeze(0), return_encodings=True, full=False
        )
        # cX_SSE_list = assign_secondary_structures(
        #     cX_coords, return_encodings=False, full=False
        # )
        cX_SSE_onehot = assign_secondary_structures(
            cX_coords, return_encodings=True, full=False
        )
        # px0_SSE_list = assign_secondary_structures(
        #     px0_ca[: cX_ca.shape[0], :].unsqueeze(0), return_encodings=False, full=False
        # )

        px0_SSE_onehot = assign_secondary_structures(
            px0_ca[: cX_ca.shape[0], :].unsqueeze(0), return_encodings=True, full=False
        )
        rmsd, _ = calc_rmsd(np.array(cX_ca), np.array(px0_ca[: cX_ca.shape[0], :]))
        # print(SSE_list)
        xt_pct_ss = torch.sum(xt_SSE_onehot, dim=1).squeeze(0) / cX_ca.shape[0]
        px0_pct_ss = torch.sum(px0_SSE_onehot, dim=1).squeeze(0) / cX_ca.shape[0]
        cX_pct_ss = torch.sum(cX_SSE_onehot, dim=1).squeeze(0) / cX_ca.shape[0]
        # print("xt_helix:", xt_pct_ss[0], "xt_strand:", xt_pct_ss[1])  # helix,strand
        print("px0_helix:", px0_pct_ss[0], "px0_strand:", px0_pct_ss[1])  # helix,strand
        print("cX_helix:", cX_pct_ss[0], "cX_strand:", cX_pct_ss[1])  # helix,strand
        print(f"rmsd:{rmsd}")

    a_1 = (torch.sqrt(alphabar_schedule[t_idx - 1] + eps) * beta_schedule[t_idx]) / (
    ...
    if method == 1:
        ...
        if cX_pct_ss[0] - px0_pct_ss[0] <= 0:
            ...
            # c_1 = b_1 / lamda_2
            # method 2
            f = lamda_1 * (cX_pct_ss[0] - px0_pct_ss[0])
            print(f"f:{f}")
            if lamda_0 == 1 or lamda_2 == 0 and lamda_1 == 0:
                c_1 = 0
            else:  # f>0
                c_1 = b_1 / (f + lamda_2)
    elif method == 2:
        ...
        if t_idx < time:
            ...
            f = lamda_1 * (cX_pct_ss[0] - px0_pct_ss[0])
            print(f"f:{f}")
            if lamda_2 == 0 and lamda_1 == 0:
                c_1 = 0
            else:  # f>0
                c_1 = b_1 / (f + lamda_2)
    elif method == 4:
        print("2")
        c_1 = b_1 / (lamda_1 * (cX_pct_ss[0] - px0_pct_ss[0]) + lamda_2)
    elif method == 5:
        print("scaffolding")
        # lamda_0 = 2
        c_1 = b_1 * lamda_0 / lamda_1
    elif method == 6:
        print("binder")
        # lamda_0 = 2
        c_1 = b_1 * lamda_0 / lamda_1
    ...
    print(f"a:{a_1}")  # 0.1->0.53
    print(f"b:{b_1}")  # 0.9->0.47
    print(f"c:{c_1}")  # 0.9
    print(a_1 + b_1 + c_1)
    # print(f"pX0_ca:{px0_ca[0]}")
    # print(f"xt_ca:{xt_ca[0]}")
    # print(f"cX_ca:{cX_ca[0]}")
    mu = a + b
    if cX_ca is not None:
        print(f"cX_ca not None")
        # cX_ca = torch.cat([cX_ca, torch.zeros_like(mu)], dim=1)
        if method == 5:
            # scaffolding
            cX_ca_copy = cX_ca
            cX_ca = np.zeros((mu.shape[0], 3))
            cX_ca[~diffusion_mask] = cX_ca_copy
            # cX_ca = np.concatenate(
            #     (cX_ca, np.zeros((mu.shape[0] - cX_ca.shape[0], 3))), axis=0
            # )
        else:
            cX_ca = np.concatenate(
                (cX_ca, np.zeros((mu.shape[0] - cX_ca.shape[0], 3))), axis=0
        ...
        assert (
            cX_ca.shape == mu.shape
        ), f"cX_ca shape is {cX_ca.shape} and mu shape is {mu.shape}"
        c = c_1 * cX_ca  # add / 2
        mu = mu + c
        # mu = torch.tensor(cX_ca)
        # cX_ca = cX_ca
    # mu 和 sigma是平移向量去噪中的 u和 β
    # visualize
    # os.makedirs("visual", exist_ok=True)
    # scores_filepath = os.path.join("visual/", "4.csv")
    # if t == 50:
    #     with open(scores_filepath, "w") as file:
    #         columns = ["t", "rmsd", "helix_diff"]
    #         file.write(",".join(columns) + "\n")
    # with open(scores_filepath, "a") as file:
    #     file.write("{},{:.3f},{:.3f}\n".format(t, rmsd, cX_pct_ss[0] - px0_pct_ss[0]))
...
class Denoise:
    ...
    def align_to_xt_motif(self, px0, xT, cx, diffusion_mask, eps=1e-6):
        """
        ...
        # cx_ = cx @ R
        # cx_ = cx_ + xT_motif_mean
        return torch.Tensor(px0_), torch.Tensor(cx)
    ...
    def get_next_pose(
        ...
        if align_motif and diffusion_mask.any():
            px0, cX_ca = self.align_to_xt_motif(px0, xt, cX_ca, diffusion_mask)
            cX_ca = cX_ca.to(xt.device)
            # print(f"cX_ca:{cX_ca}")
            # print(f"px:{px0}")
        ...
        # frames_next:(L, 3, 3) set of backbone coordinates(N,Ca,C) with slight rotation
        ...
        # print(f"ca_deltas: {ca_deltas[0]}")
        # print(f"ca_deltas[:, None, :]: {ca_deltas[:, None, :][0]}")
        # add the delta to the new frames
        frames_next = torch.from_numpy(frames_next) + ca_deltas[:, None, :]  # translate
        # frames_next[x]第一列加ca_deltas[x][0]
        # print(f"frames_next: {frames_next[0]}")
...
def sampler_selector(conf: DictConfig):
    if conf.scaffoldguided.scaffoldguided:
        ...
        elif conf.inference.model_runner == "CurveConditioning":
            sampler = model_runners.CurveConditioning(conf)
...
def process_target(pdb_path, parse_hetatom=False, center=True):
    ...
    if not center:
        ca_center = 0
    print("centering:", ca_center)
    xyz = torch.from_numpy(target_struct["xyz"] - ca_center)
    # xyz = torch.from_numpy(target_struct["xyz"])  # changed
