...
class Sampler:
    ...
    def sample_init(self, return_forward_trajectory=False):
        ...
        # print(f"binderlen: {self.contig_map.length}")
...
class CurveConditioning(Sampler):
    """
    Model Runner for curve conditioning
    ...
    def __init__(self, conf: DictConfig):
        ...
        """
        super().__init__(conf)
        # self.cX_ca = self.Read_cX_ca(self._conf.inference.cX_path)
        self.cX_ca = self.Read_cX_ca(
            self._conf.inference.cX_path, self._conf.inference.offset
        )
        self.method = self._conf.inference.method
        self.time = self._conf.inference.time
        self.lamda_0 = self._conf.inference.lamda_0
        self.lamda_1 = self._conf.inference.lamda_1
        self.lamda_2 = self._conf.inference.lamda_2

    def extract_ca_atoms(self, pdb_file: Path) -> List:
        """Extracts all C-alpha atoms from a PDB file"""
        parser = PDBParser()
    ...
    def mask_prob(self, atoms: List, prob: float) -> np.array:
        """Mask atoms with a given probability"""
        # print(np.array(atoms).shape, "wow")
        return np.array([atom for atom in atoms if random.random() > prob])

    def coords_spline(self, coords: np.array, mask_prob: float, ss: List[float] = [5]):
        """Plots coordinates in 3D space with rainbow spectrum colors"""
        spline_coords_list = []
        for s in ss:
            tck, u = splprep(coords.T, s=s)
            spline_coords = splev(np.linspace(0, 1, 36), tck)
            spline_coords_list.append(spline_coords)
        # Average the coordinates from different s
        avg_spline_coords = np.mean(spline_coords_list, axis=0)
        return np.array(avg_spline_coords).T

    def write_pdb(self, coords: np.array, pdb_file: Path):
    ...
    def Read_cX_ca(self, cX_path, offset):
        """
        Read cX_ca_atoms from file
        """
        if not cX_path:
            return np.zeros((14, 3))
        filename = os.path.basename(cX_path)
        if filename.endswith(".pdb"):
            pdb_file = Path(cX_path)
            ca_coords = self.extract_ca_atoms(pdb_file)
            np.save(f"temp.npy", ca_coords + offset)
            ca_coords = np.load(f"temp.npy")
        elif filename.endswith(".npy"):
            ca_coords = np.loadtxt(cX_path, delimiter=",")
            ca_coords = ca_coords + offset
        prob = 0.0
        # masked_coords = self.mask_prob(ca_coords, prob)
        # spline_coords = self.coords_spline(masked_coords, prob)
    ...
    def sample_step(self, *, t, x_t, seq_init, final_step):
        ...
        B, N, L = xyz_t.shape[:3]
        # print(f"xt: {x_t[0]},{x_t[1]},{x_t.shape}")
        # print(f"xt_in: {xt_in[0][0]},{xt_in.shape}")
        # print(f"xyz_t: {xyz_t[0][0][0]},{xyz_t.shape}")
        ##################################
        ######## Str Self Cond ###########
        ##################################
...
class SelfConditioning(Sampler):
    ...
    def sample_step(self, *, t, x_t, seq_init, final_step):
        ...
        ######## Str Self Cond ###########
        ################################## 
        if (t < self.diffuser.T) and (t != self.diffuser.partial_T):
            zeros = torch.zeros(B, 1, L, 24, 3).float().to(xyz_t.device)
            xyz_t = torch.cat(
                (self.prev_pred.unsqueeze(1), zeros), dim=-2
            )  # [B,T,L,27,3]
